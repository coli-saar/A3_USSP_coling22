...
def cluster_result_to_acc(cluster_counts, n_gt_clusters):
    ...
    cluster_counts: dict[cluster_index:int, dict[label:int, n_instances:int]]
        the keys go from 0 to n; the labels might be a random set. the dict represents the components of each predicted
        cluster.
    """
...
def acquire_induced_metrics(predictions, gt_labels, n_gt_clusters, index_p2g, vocab=None, scenario=None,
                            coref_modifier=None):
    # cluster_counts: dict[cluster_index:int, dict[label:int, n_instances:int]]
    cluster_counts = dict()
    ...
    ''' the prediction of clustering after mapping to the GT labels '''
    ...
    if coref_modifier:
        macro_f1, micro_f1, detail_f1 = F1(np.array(predictions_in_labels), gt_labels, index_p2g,
                                           extra_predictions=coref_modifier[scenario])
    else:
        macro_f1, micro_f1, detail_f1 = F1(np.array(predictions_in_labels), gt_labels, index_p2g)
    return{
        'acc': induced_acc,
        ...
        'micro_f1': micro_f1,
        'detail_f1': detail_f1,
        'prediction_in_labels': predictions_in_labels,
    }


def F1(predictions, gt_labels, index_mappings_p2g=None, extra_predictions=None):
    """
    extra_predictions: Dict[str, list]. class_label:list of contexts
    """
    ...
    if extra_predictions:
        n_protagonist = max([len(clist) for clist in extra_predictions.values()])
        n_predictions = sum([len(clist) for clist in extra_predictions.values()])
        by_cluster_metrics[-1] = {'truePositive': n_protagonist, 'predicted': n_protagonist, 'existed': n_predictions}
